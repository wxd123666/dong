  ...
  <div class="index-shopInfo_1RRTZ0d">
   <div class="logo-container_XoH2Vit_0">
    <div class="logo-main_21aInWJ_0">
     <img :alt="restaurant.name" class="logo-logo_3S1eSkn_0"  @click="gotoInfo(restaurant.restaurant_id)" :src="'http://localhost:8090/api/image/'+restaurant.image_path" />
     <!---->
     <!---->
    </div>
    <!---->
   ...
   <div class="index-main_N3FfcSz">
    <section class="index-line_2-iYR1A" @click="gotoMenu(restaurant.restaurant_id)">
    ...
    <section class="index-line_2-iYR1A">
     <div class="index-rateWrap_39dWx_g">
      <span class="index-rate_WsK58g8" @click="gotoRate(restaurant.restaurant_id)">{{restaurant.rating}}</span>
...
export default {
  ...
  methods: {
    gotoMenu(id){
      router.push(`/detail/${id}/menu`);
    },
    gotoInfo(id){
      router.push(`/detail/${id}/info`);
    },
    gotoRate(id){
      router.push(`/detail/${id}/rate`);
    },
    testAbc(){
      this.$store.dispatch('abc', 3);
    }
  },
  filters: {
    cn_distance (dis) {
