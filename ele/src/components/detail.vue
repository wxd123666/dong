    ...
    <div class="app-1TQsf_1">
      <div class="app-dps8r_1">
        ...
        <div class="shop-tab-1MgBk">
          <div v-for="json,index in head_titles" :key="index" :class="type==json.type?'shop-tab-2ipt1 shop-tab-nD6jp':'shop-tab-2ipt1'" @click="fnTabClick(json.type)">
           <span class="shop-tab-2H0qT">{{json.title}}</span>
          </div>
         </div>
         <menuView :id="id" v-if="type=='menu'"></menuView>
         <commentView v-if="type=='rate'"></commentView>
         <infoView v-if="type=='info'"></infoView>
      </div>
    </div>
  </div>
</template>
